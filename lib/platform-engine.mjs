...
export function analyzePlacements(placements) {
  return placements.map((placement) => {
    ...
    if (placement.ivtScore >= 60) {
      decision = "PAUSE";
      reason = "High IVT risk";
    } else if (placement.spend >= 100 && roas < 0.65) {
      decision = "PAUSE";
      reason = "ROAS below stop-loss";
    } else if (placement.ivtScore >= 30 || roas < 1) {
      decision = "WATCH";
      reason = placement.ivtScore >= 30 ? "Elevated IVT risk" : "ROAS below break-even";
    } else if (roas >= 1.5 && placement.ivtScore < 30) {
      decision = "SCALE";
...
export function summarizePlatform(placements) {
  const analyzed = analyzePlacements(placements);
