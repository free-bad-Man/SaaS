...
const defaultModules = [
  ...
  ["06", "DSP Connectors", "SYNCED", "4 platforms"],
] as const;

const connectors = [
  ["DV360", "Healthy", "12 sec ago", "Campaign stats + actions"],
  ["Google Ads", "Healthy", "28 sec ago", "Conversions + spend"],
  ["Taboola", "Healthy", "41 sec ago", "Native inventory + actions"],
  ["Custom DSP", "Review", "2 min ago", "OpenRTB logs + reporting"],
] as const;
...
type PipelineDecision = { campaignId: string; placementId: string; sourceId: string; impressions: number; clicks: number; spend: number; revenue: number; conversions: number; roas: number; cpa: number | null; ivtScore: number; decision: string; reason: string };
...
export default function PlatformConsole({ embedded = false }: { embedded?: boolean }) {
  ...
  const moduleRows = useMemo(() => pipelineResult ? [
    ...
    ["06", "DSP Connectors", "SYNCED", `${pipelineResult.modules.connector.id} · ${pipelineResult.modules.connector.mode}`],
  ...
  async function runApiPipeline() {
    ...
    try {
      const response = await fetch("/api/platform/run", {
        method: "POST",
      ...
      setResultConnector(pipelinePayload.connector);
      setPipelineState("complete");
      if (activeProjectId) setRunHistory(await fetchRunHistory(activeProjectId));
    } catch {
    ...
    <Root className={`platform-page${embedded ? " platform-embed" : ""}`} aria-label={embedded ? "Interactive 3VE.4 platform dashboard" : undefined}>
      {!embedded ? <header className="platform-topbar">
        <div className="platform-shell platform-nav">
          ...
          <div className="platform-product"><i /> ADTECH CONTROL PLANE <small>SYNTHETIC DEMO</small></div>
      ...
      <div className={`platform-shell platform-layout${embedded ? " platform-layout-embedded" : ""}`}>
        <aside className="module-rail" aria-label="Platform modules">
          ...
          {moduleRows.map(([number, title, status, metric]) => (
            <article key={number}>
              ...
              <i className={status === "BETA" || status === "SYNCED" ? "status-watch" : ""}>{status}</i>
            </article>
          ))}
        ...
        <section className="console-main">
          <div className="console-heading">
            <div><p>PROGRAMMATIC OPERATIONS</p><h1>Unified campaign control</h1><span>One synthetic workspace across ingestion, attribution, traffic quality, and media buying.</span></div>
            <button type="button" className={pipelineState === "complete" ? "is-queued" : ""} disabled={pipelineState === "running"} onClick={runApiPipeline}>{pipelineState === "running" ? "Running server pipeline…" : pipelineState === "complete" ? "Pipeline complete ✓" : "Run API pipeline →"}</button>
          </div>
          ...
          <section className="run-history-card" aria-label="Recent pipeline runs">
          ...
          {activeTab === "Overview" ? (
            <>
              <div className="platform-metrics">
                <article><span>Media spend</span><strong>{money(visibleSummary.spend)}</strong><small>{pipelineResult ? "processed input" : "synthetic period"}</small></article>
                <article><span>Attributed revenue</span><strong>{money(visibleSummary.revenue)}</strong><small>postback matched</small></article>
                ...
                <article><span>At-risk spend</span><strong>{money(visibleSummary.atRiskSpend)}</strong><small className="metric-risk">review before action</small></article>
              </div>
          ...
          {activeTab === "Policy" ? (
            <section className="policy-editor-card">
              ...
              <div className="policy-fields">
                ...
                <label><span>Minimum spend <small>USD</small></span><input type="number" min="0" step="1" value={policy.minSpend} onChange={(event) => updatePolicyNumber("minSpend", event.target.value)} /></label>
          ...
          {activeTab === "Connectors" ? (
            <div className="connector-grid">
              {connectors.map(([name, status, sync, scope]) => <article key={name}><div><span>{name.slice(0, 2).toUpperCase()}</span><b>{name}</b></div><i className={status === "Review" ? "connector-review" : ""}>{status}</i><p>{scope}</p><small>Last sync · {sync}</small></article>)}
            </div>
          ) : null}

          {activeTab !== "Connectors" && activeTab !== "Policy" ? (
            <div className="placement-card">
              <div className="console-card-title"><div><b>{activeTab === "Optimizer" ? "Decision queue" : activeTab === "Attribution" ? "Attributed performance" : "Source performance"}</b><span>Six synthetic placements</span></div><small>USD · CURRENT PERIOD</small></div>
                ...
                <table className="placement-table">
                  ...
                  <tbody>{visiblePlacements.map((placement) => (
                    <tr key={placement.id}>
                      ...
                      <td>{money(placement.spend)}</td><td>{money(placement.revenue)}</td><td>{placement.conversions}</td><td>{placement.cpa === null ? "—" : money(placement.cpa)}</td><td><b>{placement.roas.toFixed(2)}x</b></td><td>{placement.ivtScore}</td>
